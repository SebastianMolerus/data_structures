...
public:
  circular_buffer() { static_assert(Size > 0, "Buffer without memory"); }
  ...
  bool
  get(elem_type &elem,
      std::chrono::duration<long, std::milli> const timeout = time_limit_ms) {
    std::unique_lock<std::mutex> lk{m_mut};
    bool const res =
        m_cv.wait_for(lk, timeout, [this] { return m_elements_count > 0; });
    if (!res)
      return false;
  ...
  get(std::chrono::duration<long, std::milli> const timeout = time_limit_ms) {
    std::unique_lock<std::mutex> lk{m_mut};
    bool const res =
        m_cv.wait_for(lk, timeout, [this] { return m_elements_count > 0; });
...
private:
  static constexpr std::chrono::duration<int, std::milli> time_limit_ms{
